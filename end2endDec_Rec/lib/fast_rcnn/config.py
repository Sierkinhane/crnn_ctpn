...
def _merge_a_into_b(a, b):
    ...
    for k, v in a.items():
        ...
        # k是key b[k] 即是访问b中字典的value
        old_type = type(b[k])
        #a中的value与b中的value类型进行比较
        if old_type is not type(v):
            # 判断b[k]是否为 np.ndarray类型
...
def cfg_from_file(filename):
    ...
    import yaml
    # filename:'ctpn/text.yml'
    with open(filename, 'r') as f:
        # 加载yaml文件
        # 用easydict包装text.yml文件 使得可以 .key 访问字典内容
